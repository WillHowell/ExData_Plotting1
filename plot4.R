...
#set up plot area for two rows, 2 columns
par(mfrow=c(2,2))

#upper left
plot(filtered$datetime,filtered$Global_active_power,type="l",ylab="Global Active Power", xlab="")

#upper right
plot(filtered$datetime,filtered$Voltage,type="l",ylab="Voltage", xlab="datetime")
...
legend("topright", c("Sub_metering_1","Sub_metering_2","Sub_metering_3"), lty=c(1,1), 
       col=c("black","red","blue"),cex=0.8, ncol=1,bty="n") 

#lower right
plot(filtered$datetime,filtered$Global_reactive_power,type="l",ylab="Global_reactive_power", xlab="datetime")
